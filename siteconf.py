"""siteconf -- where am I, and what does this cluster want.
    ...
    siteconf.SWEEP_DIR, .SCRATCH, .DATA_DIR, .SETUP_COMMANDS, .CLUSTER
...
SITE_KEYS = ("partition", "account", "qos", "gres", "gpu_flag", "constraint", "max_cpus_per_gpu")
...
def resolve(cfg):
    ...
    scheduler directives. Keys already present are NOT overridden, so a config
    that deliberately pins something still wins.
    """
    paths = cfg.get("paths") or {}
    cfg["paths"] = paths
    paths.setdefault("project_dir", _S["project_dir"])
    # a config may name a sweep_dir under the old root; re-root it here
    ...
    for k in ("afs_sweep_dir", "eos_sweep_dir"):
        paths.pop(k, None) if CLUSTER.get("scheduler") != "htcondor" else None
...
def slurm_header(cluster, job_name, out, err, extra=()):
    ...
    clamps cpus to the site's per-GPU ceiling, and never names a partition or
    account that isn't this cluster's.
    """
...
def cpu_header(job_name, out, err, cpus=32, time="04:00:00", extra=()):
    ...
    lines = ["#!/bin/bash", "#SBATCH --job-name=%s" % job_name,
             "#SBATCH --partition=%s" % CPU.get("partition", CLUSTER.get("partition")),
             "#SBATCH --account=%s" % CPU.get("account", CLUSTER.get("account")),
             "#SBATCH --nodes=1", "#SBATCH --ntasks=1",
             "#SBATCH --cpus-per-task=%d" % int(cpus)]
