...
\begin{itemize}
  ...
  \item \textbf{Massive-particle thresholds and mixed coverage} (the remaining harder-divergence cases;
    the multi-scale $u\bar u g$ and more-legs $u\bar u ggg$ tests are \emph{done} --- see the
    ``concentration'' principle above). The coordinate-free loop has only been stressed on \emph{massless}
    final states so far. Still open: massive-particle thresholds ($t\bar t$ near threshold, the $2m_t$
    cusp) and a single process carrying soft$+$collinear$+$resonant coverage at once. The generator
    (\code{gen\_ir\_democratic.py}) already handles arbitrary massless $N$-body, but the threshold cusp is
    a \emph{massive} scale the democratic split does not target --- it needs mass-aware proposal support
    and a per-process standalone label. The open sub-question there mirrors the concentration principle:
    is a threshold cusp ``concentrated'' enough for $\sigma$-steering to help, or diffuse like $u\bar u ggg$?
