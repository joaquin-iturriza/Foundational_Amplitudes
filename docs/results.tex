...
uniformly-hard set cannot help. Figures \code{l2\_uug\_multiscale} (per-decade $+$ $\sqrt s$-region panel),
\code{l2\_uuggg\_perdecade}.
...
claim is ``no evidence BBB beats the het head'', and ``BBB beats uniform'' is marginal. Figure
\code{l2\_bbb\_sweep}; tooling \code{bbb.py}, \code{sweep/l2\_bbb\_trial.py},
\code{sweep/l2\_bbb\_sweep\_config.yaml} (the sweep drives the L2 loop via \code{generate\_sweep}'s new
\code{trial\_script} hook).
