...
data, $\eta^\ast\propto D^{+0.15\ldots0.3}$, and \emph{saturates} ($\sim2\times$ over $100\times$
data). The optimum is thus a 2D surface $\eta^\ast(t,D)$ --- approximately the inverted-$U$ in
$t$ (Eq.~\eqref{eq:invU}) times this weak, saturating factor in $D$ --- and both axes are set by
the experimenter when the run is designed. \textbf{Set the learning rate by evaluating the
surface at your own $t$ and $D$}; there is no ``regime'' to diagnose. Every real run sits well
past the peak ($t\gg\tstar$), so it is on the decay side and a \emph{longer} run takes a
\emph{lower} centre, computed from its $t$. A fixed-epoch scaled run has $D\propto t$, so the net
exponent past the peak is $t^{-0.55+0.15\ldots0.3}=t^{-0.40\ldots-0.25}$: the $D$-slope
\emph{softens} the drop but does not cancel it. Because that $D$-factor is weak and saturating,
it only lifts the fixed-data centre $\eta_c(t)$ by $<2\times$ --- already inside the
$\pm\tfrac12$-decade sweep window --- so in practice one centres on $\eta_c(t)$ and lets the
window absorb the $D$-nudge. (An earlier ``is $\vlnr$ still dropping vs.\ floored?'' rule of thumb
is dropped: that distinction was never a measured axis --- only $t$ and $D$ were --- and it is
unnecessary once both are read off the run design.)
\end{remark}
...
sampled compute range (Fig.~\ref{fig:perdataset} shows the per-process learning curves for all
pretraining processes). A virtual/Born \emph{ratio} target was also tried as a learning
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{per_dataset_scaling.pdf}
  \caption{\textbf{Per-process scaling of the joint foundation model.} Held-out loss vs.\ training
  compute for each pretraining process, with the fitted floor-aware law Eq.~\eqref{eq:scaling}
  overlaid; the local slope is the exponent $\alpha_C$ of Table~\ref{tab:scaling}, steeper for
  lower final-state multiplicity.}
...
\begin{figure}[H]
  ...
  \caption{\textbf{Compute-scaling exponent vs.\ final-state multiplicity.} From-scratch (blue)
  and full-finetune (red) fitted $\alpha_C$ against the theoretical lower bound
  $\alpha=4/\mathrm{DOF}$ (grey). $2\!\to\!2$ targets sit near the bound; higher multiplicity
  falls below it. Fine-tuned targets ($2\!\to\!2$) match the from-scratch exponent --- the
  transfer advantage is in the prefactor and floor, not the slope (\S\ref{sec:transfer}).}
  \label{fig:alphamult}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.49\textwidth]{transfer_nh8.pdf}\hfill
  \includegraphics[width=0.49\textwidth]{finetune_methods_walltime.pdf}
  \caption{\textbf{Transfer.} Left: held-out loss vs.\ compute and wall-time for fine-tune (from
  the 8- and 25-process foundations) vs.\ from-scratch on $ee\,u\bar u$ and $ee\,t\bar t$
  NLO-virtual --- fine-tuning stays below solo and does not plateau. Right: the five fine-tune
  methods in wall-time; full layer-decay reaches a given loss soonest.}
  \label{fig:transfer}
\end{figure}
...
$\eta_c(t)=10^{-2}\min[(t/3000)^{0.5},(t/3000)^{-0.55}]$ (Eq.~\ref{eq:invU}) \emph{evaluated at
your actual horizon $t$}, and sweep only $\pm\tfrac12$ decade, not a flat 4-decade prior. Every
real run sits past the peak ($t\gg\tstar=3$k), so $\eta_c$ is on the $t^{-0.55}$ branch and
\emph{decreases} with horizon --- a longer run takes a lower centre (e.g.\ $\approx$2.8, 2.1,
1.5$\times10^{-3}$ at $t=$30, 50, 100k), never the $\tstar$ peak value. A scaled run's weak,
saturating $D$-slope lifts that centre by $<2\times$, already inside the window; it is not a
separate regime to diagnose.
\end{principle}
