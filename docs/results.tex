...
uncertainty}\label{sec:div-handoff}
...
\mathrm{diff}}$), consistent with the interior mixture. \textbf{Open:} merge \code{heterosc} into
\code{jeanzay}; integrate the CPU-validated online importance-sampler prototype
(\code{adaptive\_ir.py}, branch \code{adaptive-ir}) into the training loop; extend the soft/collinear
split to $u\bar u gg$. Tooling under \code{analysis/divergences/}; heteroscedastic code on worktree
\code{worktrees/wt-heterosc} (branch \code{heterosc}).}
...
un-regenerable.} (Cleanup note for the \code{heterosc}$\to$\code{jeanzay} merge: the HETEROSC val
``combined'' display still prints $0$ via geometric mean --- harmless, since training/checkpointing use
the arithmetic mean.)
