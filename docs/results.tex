...
\subsection{Hand-off: deeper/cleverer sampling, optimal allocation, and heteroscedastic
uncertainty (work in progress)}\label{sec:div-handoff}

\emph{State of an in-progress line of work, now four threads. Thread~A is complete; Thread~B is
analysis plus a validated but un-integrated tool; Thread~C (heteroscedastic $\sigma$) is
\textbf{de-risked} --- the coupled-fit failure is resolved by a two-stage recipe, and $\sigma$ is
validated as a reweighting signal (a speed and a tolerance study, below); Thread~D is the
synthesis that reframes the whole line of work around a general, process-agnostic default
sampling. New tooling is under \code{analysis/divergences/}; the heteroscedastic code lives on git
worktree \code{worktrees/wt-heterosc} (branch \code{heterosc}), \emph{not yet merged} to
\code{jeanzay}. The adaptive-sampler prototype is on branch \code{adaptive-ir}.}
...
\paragraph{Next step.} The L0+L1 picture is complete and consistent: \emph{generate a modest
flat-$\log|\mathcal{M}|^2$ admixture ($f\!\approx\!0.25$--$0.5$) for coverage; add $\sigma$-reweighting only
when the pool is starved and un-regenerable.} Merge \code{heterosc} into \code{jeanzay} (cosmetic
left-over: the HETEROSC val ``combined'' display still prints $0$ via geometric mean --- harmless,
training/checkpointing use arithmetic mean). Earlier follow-up still open: extend the soft/collinear split
to $u\bar u gg$ (\code{holdout\_prep.py} \code{region} already supports it).
