...
\begin{figure}[H]
  ...
  \caption{\textbf{Thread A: the antenna/uniform mixture has a broad interior optimum.}
  \emph{Left:} overall held-out error vs the antenna fraction $f$. The two curves are the \textbf{same
  runs on the same test set} and differ only in how the per-event errors are \emph{aggregated}: red
  weights every $y_{\min}$ decade equally (our objective), blue weights every \emph{event} equally.
  Blue sits lower because the test set is antenna-generated and hence deep-IR-dense ($\sim\!50\%$ of
  events below $y_{\min}\!=\!10^{-3}$), so event-weighting lets the deep IR set the number; the two
  disagree most at $f{=}0$ ($0.34$ vs $0.11$), precisely where the error is most uneven across decades,
  and converge once it is flat. They agree on the basin. Both endpoints are bad --- $f{=}0$ (pure
  \textsc{rambo}) is catastrophic ($\sim\!200\times$) and $f{=}1$ (pure antenna) is $\sim\!2\times$ worse
  than the basin because it starves the $O(1)$ bulk --- while any $f\in[0.25,0.75]$ (shaded) sits in a
  flat basin whose fine location is single-seed noise. \emph{Right:} the same runs resolved per
  $y_{\min}$ decade, which shows the mechanism: $f{=}0$ is steeply bad in the deep IR and best in the
  bulk, $f{=}1$ is the mirror image, and the interior mixtures are flat across all decades. This ``mix
  the coverage criterion in at a modest fraction, do not adopt it as the base'' shape recurs for the $Z$
  resonance (Fig.~\ref{fig:eeuu_genmix_fraction_sweep}).}
...
\begin{figure}[H]
  ...
  \caption{\textbf{L0 on the $ee\to u\bar u$ $Z$ resonance: generation, not resampling.} Error across
  $\sqrt s$ for the raw \textsc{rambo} base vs a flat-$\log|\mathcal M|^2$ arm. Flat \textsc{rambo} puts
  only $0.4\%$ of events in $\sqrt s\!\in\![88,95]$, and the foundation fits the peak $\sim\!400\times$
  worse than the bulk. \emph{Resampling} the fixed $10^6$ pool cannot fix this --- the pool holds only
  $\sim\!3300$ true pole events, so flat resampling just duplicates them ($155$k unique of $400$k). Only
  \emph{generating} fresh pole events lifts coverage $0.4\%\!\to\!32\%$ with genuinely new physics. This
  is the coverage ceiling of Thread~D made concrete: you cannot resample past what the pool contains.}
...
\begin{figure}[H]
  ...
  Error vs $f$, the fraction of freshly \emph{generated} flat-$\log|\mathcal M|^2$ events (rest
  uniform-$\sqrt s$), same base8/HPs/$N{=}400$k. Under the equal-per-$\sqrt s$-decade metric:
  $1.0\mathrm{e}{-}3$, $3.5\mathrm{e}{-}5$, $\mathbf{1.5\mathrm{e}{-}5}$, $7.9\mathrm{e}{-}5$,
  $1.1\mathrm{e}{-}4$ at $f=0,0.25,0.5,0.75,1$. The interior optimum beats raw \textsc{rambo}
  $\sim\!30$--$70\times$ and \emph{pure} flat $\sim\!4$--$7\times$: flat-$\log|\mathcal M|^2$ is a
  criterion to \emph{mix in}, not a good base --- pure flat starves the bulk exactly as \textsc{rambo}
  starves the pole. Same shape as the $u\bar u g$ antenna sweep
