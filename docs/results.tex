...
softening of the IR, in advance; (iii) the keep rule's shape is settled --- a nested polynomial confirms
...
\code{eval\_deep.sh}, \code{plot\_deep\_sampling.py}): antenna crushes the pole --- median
relative error in $y_{\min}\!\in\![10^{-6},10^{-5})$ drops $47.9\%\!\to\!1.4\%$; overall
MSE$(\Delta\ln|\mathcal{M}|^2)$ $0.108\!\to\!0.0017$ ($\sim$62$\times$). The fraction sweep
...
$|\mathcal{M}|^2$; $\sqrt{s}$ is eval-only. Generation lifts pole coverage $0.4\%\!\to\!32\%$ with
GENUINE new events. \emph{Bug found + fixed (would have inverted the result):} the standalone expects
...
continuum \emph{untouched} ($8.6\times10^{-4}$, unchanged) --- overall MSE $2.2\times10^{-2}\to
...
for $u\bar u gg$, reversal for $u\bar u ggg$ --- is the sharpest evidence for the principle below
(Fig.~\ref{fig:l2_gamma_response_extended}).
...
standalone slot-order check were required (the convention is per-build; $b\bar b g$ was fixed by the sign
of $A_{\rm FB}$ at the $Z$ pole against the validated $u\bar u g$). \emph{What makes this more than one
...
$0.82\times$ --- overall $2.15\!\to\!1.29\times10^{-2}$ ($0.60\times$), log-flat $0.80\times$. So the gain
...
already-learned IR continuum alone) --- and gives $\sim\!$nothing when the difficulty is diffuse across the
...
\emph{pure} MSE so checkpoint selection and the comparison metric stay like-for-like). Crucially the model
...
--- its own HP search runs the posterior width to the deterministic bound to recover that accuracy. The cheap
...
\begin{itemize}
  ...
  \item \textbf{The $\mu$P-LLoCa $\sigma$-head subset is now on \code{jeanzay}} (cherry-picked, not a
    ...
    ``combined'' display still prints $0$ via the geometric mean --- harmless, since training and
    checkpoint selection use the arithmetic mean.
  \item \textbf{Massive final states} (the remaining harder-divergence case; the multi-scale $u\bar u g$
    and more-legs $u\bar u ggg$ tests are \emph{done} --- see the ``concentration'' principle above).
    \emph{The $t\bar t$ threshold is ruled out as the test, and this is a real finding rather than a
    deferral:} at tree level $ee\to t\bar t$ has \textbf{no threshold divergence at all} --- $|\mathcal
    M|^2$ spans only $8.7\times$ in total and is \emph{flattest} at threshold ($\mathrm{std}\log|\mathcal
    M|^2 = 0.022$ for $\beta<0.05$ vs $0.665$ in the bulk). The $2m_t$ cusp is a phase-space/cross-section
    feature, not an amplitude one, so running it would have produced a guaranteed null that says nothing
    about the method. The replacement is $ee\to b\bar b g$ (registry row added): the direct massive
    analogue of $u\bar u g$ --- same $Z$ resonance, same soft-gluon singularity --- but with the
    \emph{collinear} limit regulated by $m_b$, and the dead-cone scale $m_b^2/s\!\approx\!2\times10^{-5}
    $--$3\times10^{-3}$ sits \emph{inside} the $y_{\min}$ window we bin over, so the mass reshapes the
    divergence instead of merely relabelling it ($|\mathcal M|^2$ spans $7\times10^{10}$). Generator
    support turned out to be already present --- \code{democratic\_draw} produces exact on-shell massive
    momenta (verified to $10^{-13}$) and \code{fiducial\_pass\_mask} already skips massive legs; what was
    missing was only the registry row, a mass-correct collinear variable ($2p_i\!\cdot\!p_j/s$ rather than
    $s_{ij}/s$, identical for massless legs), and the standalone slot-order check. \emph{The run is now
    done (single seed):} $\sigma$-steering on $b\bar b g$ localises to the $Z$ peak ($0.56\times$) with a
    weak IR gain ($0.85\times$), both predicted in advance by the base-arm $\sigma$-contrast --- see the
    massive-process paragraph and Fig.~\ref{fig:l2_bbbarg} above. \emph{Still genuinely open:} a $3$-seed
    repeat of $b\bar b g$, and a single process carrying soft$+$collinear$+$resonant coverage at once.
    \textbf{Slot-order trap, now characterised:} the convention is \emph{per-build}, not global ---
    \code{ee\_ttbar} takes production order bit-identically while \code{ee\_uu} needs its final pair
    swapped --- so every new standalone must be checked. Where production data exists, relabel and
    compare; where it does not (\code{ee\_bbbarg}), the sign of $A_{\rm FB}$ at the $Z$ pole against an
    already-validated standalone is a data-free substitute (both negative under the same construction,
    with $|A_{\rm FB}(b)|>|A_{\rm FB}(u)|$ as the SM requires; the swapped order anti-correlates).
  \item \textbf{The \emph{cheap} epistemic approximations, and a seed-replicated confirmation}
    (full-network Bayes-by-backprop is \emph{done} and lost --- see the ``ranking'' principle above).
    ...
    het head'' from suggestive to settled. Also untested: the claimed \emph{annealing} property --- epistemic
