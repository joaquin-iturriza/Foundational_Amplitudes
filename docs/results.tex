...
$\pi\!\propto\!\sigma^\alpha$ (no physical binning, process-agnostic); (L2, aspirational)
$\sigma$-informed \emph{on-the-fly generation} --- the only layer that lets $\sigma$ overcome the
coverage ceiling, by generating where $\sigma$ is high \emph{and} data is sparse.
...
\begin{itemize}
  \item \textbf{Merge \code{heterosc} into \code{jeanzay}.} Cosmetic left-over to clean up in the
    merge: the HETEROSC validation ``combined'' display still prints $0$ via the geometric mean ---
    harmless, since training and checkpoint selection use the arithmetic mean.
  \item \textbf{Coordinate-free $\sigma$-driven online generation (L2) --- BUILT and RUN on
    \code{ee}$\to$\code{uu}, but EVERY cross-arm number from that session is CONFOUNDED; treat all
    magnitudes as INVALID pending the matched re-run below.} Tooling lives on worktree
    \code{worktrees/wt-l2-gen} (branch \code{l2-gen}, off \code{heterosc}; \emph{not merged}, and its
    \code{results.tex} is stale --- this hand-off is the current record).
    \code{analysis/divergences/l2\_online.py} is a single-GPU-job, resumable round orchestrator
    (train $\mu$ $\to$ two-stage $\sigma$-fit $\to$ re-score a frozen $3\mathrm{M}$ candidate pool $\to$
    regenerate the training pool $\to$ repeat; each GPU stage a fresh \code{run.py} subprocess to dodge
    the $\mu$P global leak). Eval \code{eval\_l2\_online.sh}$\to$\code{eval\_eeuu\_resonance.py};
    plots \code{plot\_l2\_online.py}. \code{extract\_sigma\_eeuu.py}/\code{eval\_eeuu\_resonance.py}
    were patched to self-locate the worktree (they must load the HETEROSC \code{models.lloca}).
    \begin{itemize}
      \item \emph{Method that survives (the design, not the numbers).} The general, divergence-agnostic
        sampler is \textbf{$p(x)\!\propto\!\sigma(x)^\gamma$} (\code{adapt\_pool}): per-event, with NO
        physics coordinate (no $\sqrt s$/$y_{\min}$/$|\mathcal M|^2$ binning), NO reference density, NO
        loss correction; damped by a per-candidate $\sigma$-EMA so a large $\sigma$ ratio cannot collapse
        the whole budget onto a few events and oscillate. Iterated, it flows toward equalized $\sigma$
        (uniform error). This is the whole point of $\sigma$: it flags where the model is wrong in ANY
        process without naming the divergence variable.
      \item \emph{MISTAKE 1 --- un-matched training budget.} The ``adapt $\approx$ f=0.25'' headline
        compared \code{adapt} at $6$--$8$ rounds ($9$--$12$k $\mu$-iters) against f=0.25 at $3$ rounds
        ($4.5$k $\mu$-iters). At MATCHED budget f=0.25 is clearly ahead ($r{=}2$: $2.1\mathrm{e}{-}5$ vs
        adapt's $7.9\mathrm{e}{-}5$); adapt only draws level after spending extra rounds discovering the
        density from a uniform start. So there is NO measured tie --- at equal budget the hand-tuned
        $f$ wins; the honest open claim is only ``adapt converges TOWARD f=0.25 autonomously, paying a
        transient.''
      \item \emph{MISTAKE 2 --- data-freshness confound (taints EVERY $\sigma$-vs-static comparison this
        session).} The static/raw/reweight arms \emph{reused the same $400$k pool every round}, while the
        adaptive/score arms \emph{drew a fresh $400$k each round} --- so the ``winning'' arms also saw
        $\sim$$2$--$3\times$ more UNIQUE data. This confounds the covered-base and starved-base
        $\sigma$-vs-static runs, the ``$7.5\times$ over raw'' starved result, AND the adapt-vs-f=0.25
        comparison. Fixed now (see below), but all prior numbers predate the fix.
      \item \emph{MISTAKE 3 --- two wasted detours before the right method.} (a) An unbiased IS loss
        correction $c_b\!=\!Q_b/\pi_b$ to a FIXED $Q\!=\!$f=0.25 objective: mathematically unbiased
        (mean weight $1$, verified) but it \emph{caps} performance at f=0.25 by construction --- the
        wrong target, and it made the ``reweight'' control tie ``l2'' trivially. (b) A $\sqrt s$-binned
        water-filling allocator: particularizes a method that must be coordinate-free. Both removed.
      \item \emph{What is probably still directionally true} (unconfounded by data-freshness, or verified
        analytically): uncorrected $\sigma$-resampling HURTS --- changing which events train without a
        loss correction biases the objective, so even the perfect-error oracle loses to naive coverage;
        the IS correction removes that bias but does not fix an over-concentrated ($\alpha{=}1$, per-event)
        proposal; the coordinate-free \code{adapt} loop is numerically STABLE and CONVERGES (the
        $\sigma$-EMA damping works). No magnitude claim is trustworthy.
      \item \emph{The CORRECT experiment (fairness fix ALREADY COMMITTED on \code{l2-gen}:
        static/reweight now redraw a fresh pool each round, \code{seed+r}).} Run ALL arms identically ---
        same $R$ rounds, same $1500$ $\mu$-iters/round, same FRESH $400$k/round, same seeds, same
        candidate pool --- varying ONLY the sampling distribution \{uniform (raw), fixed f=0.25, adaptive
        $\sigma^\gamma$\}, and compare the logflat curve PER ROUND-INDEX (isolates sampling from budget;
        the $\sigma$-fit iters are frozen-$\mu$ overhead, not $\mu$-training, so they never enter the
        budget axis). $\sim$$2$ GPU-h: \code{adapt} seeds $0$--$2$ already exist and are fresh-per-round
        (reusable); only the two static baselines need re-running fresh, $6$ rounds, $3$ seeds:
        \code{l2\_online.sh --arm static --f 0.25 --tag mf025\_s\$s --rounds 6 --seed \$s} and
        \code{--arm static --f 0 --tag mraw\_s\$s --rounds 6 --seed \$s}.
      \item \emph{Open beyond the fix.} A genuine WIN (adapt beats a fixed $f$) is unlikely on this
        simple $2\!\to\!2$ resonance, where one $f\!\approx\!0.25$ is already near-optimal --- test where
        no $1$-parameter family fits: \code{uugg}, or the $u\bar u g$ soft AND collinear IR at once. Then
        generalize \code{adapt} to $u\bar u g$ with the SAME code (only $\sigma$, no $y_{\min}$) as the
        process-agnostic demonstration. Note also: a round is a warm-restarted cosine finetune, not one
        continuous schedule --- reconsider whether that is the right vehicle. Distinct from this loop, the
        per-$y_{\min}$-bin \code{adaptive\_ir.py} (branch \code{adaptive-ir}) remains CPU-validated but
        unwired.
    \end{itemize}
