...
temperature over seeds, and the $\sigma$-tail \emph{saturates} at a data-independent floor, giving a
stopping rule for how much to generate. Tooling under \code{analysis/divergences/};
...
\paragraph{Uncapped data: the uncertainty tail saturates, and it does so data-\emph{independently}.}
...
\code{plot\_saturation.py}, Fig.~\ref{fig:l2_saturation}): the $\sigma$-$p99$ falls steeply then
\textbf{floors at ${\approx}0.046$ \emph{regardless of budget}} --- $75$k reaches $0.0457$, $150$k
$0.0465$, $600$k $0.0498$ (still descending only because its larger per-round increment has taken fewer
optimizer steps to the same wall-clock). So $8\times$ more data does \emph{not} lower the floor: at this
model size and horizon the deep region is saturated by ${\sim}75$k events, and the floor is set by
capacity/horizon, not data volume. The per-round fractional fall in $p99$ crossing (and staying below) a
$2\%$ tolerance is a machine-checkable stopping rule --- the \code{--stop\_on\_saturation} latch freezes
the pool there and lets training continue --- so the loop can add data until a region saturates instead
of running to a fixed cap. This also makes the ``add more capacity, not more data'' half of the
concentration principle actionable: a floored $\sigma$-tail that no longer falls with data is the signal
to stop generating and grow the model.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{l2_saturation.pdf}
  \caption{\textbf{Saturation: the uncertainty tail converges to a data-independent floor.} \emph{(a)}
  $\sigma$-$p99$ over the fresh proposal batch vs pool size, for three data budgets on $u\bar u gg$. The
  curves do not collapse on pool (a larger budget takes fewer optimizer steps to reach a given pool, so
  its live $\sigma$ sits higher there), but all converge to the same ${\approx}0.046$ floor by end of
  training --- $8\times$ more data does not lower it, so ${\sim}75$k events already saturate the deep
  region at this model size/horizon. \emph{(b)} The per-round fractional fall in $p99$; crossing below
  the $2\%$ tolerance band is the stopping rule the \code{--stop\_on\_saturation} latch uses.}
