...
un-regenerable pool --- \emph{being tested now} (\code{sigma\_fit\_raw.sh} $\to$ 3-seed
$\sigma$-reweight of a $1.5\mathrm{M}$ \textsc{rambo} base).

\paragraph{Next step.} Read out the starved-base L1 test; if $\sigma$ helps there but not on the covered
base, that scopes L1 precisely (emphasis matters iff coverage is capped). Then merge \code{heterosc} into
\code{jeanzay} (cosmetic left-over: the HETEROSC val ``combined'' display still prints $0$ via geometric
mean --- harmless, training/checkpointing use arithmetic mean). Earlier follow-up still open: extend the
soft/collinear split to $u\bar u gg$ (\code{holdout\_prep.py} \code{region} already supports it).
