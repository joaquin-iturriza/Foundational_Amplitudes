...
\begin{itemize}
  ...
  \item \textbf{Bayesian networks, not only the heteroscedastic loss.} The current $\sigma$ is a
    single \emph{point} heteroscedastic head (aleatoric-style readout off detached features). Test a
    genuinely \emph{Bayesian} network --- a posterior over weights (variational/Bayes-by-backprop,
    MC-dropout, a deep ensemble, or a Laplace/last-layer approximation) --- so the uncertainty that
    steers generation is \emph{epistemic} (``the model has not seen data like this'') rather than a
    learned spread. Epistemic $\sigma$ is arguably the \emph{right} signal for on-the-fly generation:
    it is high exactly where the training set is sparse, which is what L2 wants to fill, and it should
    shrink as coverage improves (a natural stopping/annealing signal the point head lacks). Compare,
    on the same \code{uugg} L2 harness and held-out deep-IR metric, epistemic-$\sigma$ generation vs.\
    the het-loss $\sigma^\gamma$ baseline; watch cost (ensemble = $k\times$ training; MC-dropout /
    last-layer $\approx$ free) and whether epistemic $\sigma$ ranks the deep IR better than the
    het-head's within-region signal (which was the weak link at L1).
\end{itemize}
