...
\emph{State of an in-progress line of work, three threads. Thread~A is complete; Thread~B is
analysis plus a validated but un-integrated tool; Thread~C is mid-debug and is the main open
item. New tooling is under \code{analysis/divergences/}; the heteroscedastic code lives on git
...
\paragraph{Open --- the uug finetune underfits.} \code{finetune\_heterosc\_v2.sh} (best foundation
$\to$ uug antenna pool, $\beta{=}1$, \emph{no} head reset, both sides 2ch) still yields a
$\sigma$-map (\code{extract\_sigma.py}, fig \code{heterosc\_sigma\_map}) with $\mu$ error $\sim$1
and $\sigma$ flat $\sim$1 --- $\mu$ near the mean. This is \emph{not} a data mismatch (base22 is
genuinely leave-uug-out: its log shows ``Loaded 22 recipe processes'', and \code{data.dataset} in a
\code{source=recipes} config is vestigial, so the ``8 datasets'' there is a red herring) and
\emph{not} the val-aggregation bug (single-dataset finetunes use the base arithmetic
\code{\_validate}). The MSE finetune on the \emph{same} transfer task fits to $\sim$0.01, so
HETEROSC should too.

\paragraph{What's left (priority order).}
(1) \textbf{Fix the HETEROSC uug finetune.} Diagnose first ($\sim$0.3 GPU-h): its best-vs-last
checkpoint $\mu$-MSE (rules in/out checkpoint selection) against the MSE antenna finetune's uug
$\mu$-MSE as a matched baseline. Prime suspects: finetune HPs (\code{lr\_scale}/\code{layer\_decay}
were borrowed from the MSE recipe, never tuned for the NLL gradient scale), the $\sigma$-head
sharing the backbone, or checkpoint selection. Then a finetune HP sweep if needed.
(2) \textbf{Redo the $\sigma$-map on a well-fit $\mu$.} The earlier (contaminated) map already
suggested $\sigma$ is \emph{epistemic} --- it peaked in the antenna-\emph{undersampled} bulk, not
the float32-heavy deep IR, and sat 2--3 decades above the float32 floor. Confirm once $\mu$ fits.
(3) \textbf{Then:} drive adaptive sampling from $\sigma$ (or an epistemic variant ---
MC-dropout/evidential, since a single $(\mu,\sigma)$ head is aleatoric), optionally wiring in the
Thread~B controller; and merge \code{heterosc} into \code{jeanzay} once sound (cosmetic left-over:
the HETEROSC val ``combined'' display still prints $0$ via geometric mean --- harmless, since
training and checkpointing now use arithmetic mean).
