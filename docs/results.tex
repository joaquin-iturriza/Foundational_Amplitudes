...
\begin{itemize}
  ...
  \item \textbf{Integrate the online importance-sampler.} The per-$y_{\min}$-bin adaptive proposal
    (\code{adaptive\_ir.py}, branch \code{adaptive-ir}) is CPU-validated but not wired into the
    training loop; wire the online L0$+$L1 loop (train $\to$ fit $\sigma$ $\to$ reweight $\to$ repeat).
