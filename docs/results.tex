...
\begin{itemize}
  ...
  \item \textbf{Extend the soft/collinear split to $u\bar u gg$} (\code{holdout\_prep.py}
    \code{region} already supports it).
  \item \textbf{Harder / structurally-different divergences.} The L2 loop is coordinate-free, so it
    should transfer with \emph{no} per-process tuning --- stress it on divergence structures it has
    not seen: more legs / higher multiplicity ($u\bar u ggg$, $u\bar u q\bar q g$), a genuine
    \emph{multi-scale} overlap (simultaneous $s$-channel resonance \emph{and} IR, e.g.\ $Z$/$W$
    radiation), massive-particle thresholds ($t\bar t$ near threshold, $2m_t$ cusp), and mixed
    soft$+$collinear$+$resonant coverage in one process. The open question: does the single $\gamma$
    knob still suffice when there is more than one competing extreme, or does the density want to be
    multi-modal (does $\sigma^\gamma$ automatically split its budget, or must it)? The generator
    (\code{gen\_ir\_democratic.py}) already handles arbitrary massless $N$-body; massive/threshold
    cases need the mass handling and a standalone label per process.
