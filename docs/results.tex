...
region and we measure RMS $\Delta\log|\mathcal{M}|^2$ inside it (the evaluation
...
(RMS $0.84$, and $3.3$ in the deepest $y_{\min}<10^{-3}$ bin, where the amplitude is
mispredicted by $\sim\!e^{3}$), but adding back just \emph{five percent} of the
divergent-region pool cuts the error $4.9\times$ (to $0.17$), after which the curve
flattens toward the in-support baseline ($0.053$ at $f\!=\!1$). Every $f$ degrades
...
\hline
RMS $\Delta\log|\mathcal{M}|^2$ (all held-out) & $0.842$ & $0.173$ & $0.106$ & $0.080$ & $0.053$ \\
\quad deepest bin $y_{\min}<10^{-3}$ & $3.34$ & $0.74$ & $0.44$ & $0.32$ & $0.19$ \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  $f\!=\!0$ is pure extrapolation). Left: RMS and MAE $\Delta\log|\mathcal{M}|^2$ over
  the whole held-out region; right: split by $y_{\min}$ sub-bin (deeper IR toward the
...
events recovers $\sim\!5\times$ of the error, and the add-back curve then saturates
toward the in-support baseline.
\end{principle}

\paragraph{Follow-ups.} Separate the soft ($x_g$) and collinear ($y_{\min}$) axes;
