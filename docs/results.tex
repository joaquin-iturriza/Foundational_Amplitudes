...
\paragraph{Next step (start of next session).} Build the \textbf{flat-in-$\log|\mathcal{M}|^2$
resampler} (L0) --- the reusable, process-agnostic piece --- and validate on a \emph{different}
singularity type with no structure-aware code: $e^+e^-\!\to u\bar u$ ($91$--$1000$ GeV) has an
$s$-channel $Z$ resonance ($|\mathcal{M}|^2$ peaks $\sim$200$\times$ at $\sqrt{s}\!\approx\!91$; the
\textsc{rambo} set gives only $0.3$\% of events there --- the same starvation as the uug bulk). Show
flat-$\log|\mathcal{M}|^2$ resampling of the $10^6$-event pool fixes the resonance vs raw
\textsc{rambo} (validate against $\sqrt{s}$, which the \emph{method} never uses), then that
$\sigma$-reweighting adds emphasis on top. Then wire the online L0+L1 loop
(train $\to$ fit $\sigma$ $\to$ reweight $\to$ repeat) and merge \code{heterosc} into \code{jeanzay}
(cosmetic left-over: the HETEROSC val ``combined'' display still prints $0$ via geometric mean ---
harmless, since training and checkpointing use arithmetic mean).

\paragraph{Earlier follow-ups (still open).} Extend the soft/collinear split to $u\bar u gg$
(\code{holdout\_prep.py} \code{region}=soft/collinear already supports the split).
