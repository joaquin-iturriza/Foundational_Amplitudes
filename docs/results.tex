...
\paragraph{Thread A --- deeper + antenna phase-space sampling (done).} The production
...
well \emph{above} this (short trainings) --- optimisation-limited, not precision-limited. If ever
precision-limited, reparametrise (feed $\ln y_{\min}$ / pair masses as features) rather than store
float64.

\paragraph{Thread B --- optimal allocation (analysis + un-integrated tool).} For a fixed data
...
\paragraph{Thread C --- heteroscedastic $(\mu,\sigma)$ head (in progress; main open item).}
...
\emph{Wiring (done):} HETEROSC existed for the MLP configs but was never wired for the $\mu$P
...
\emph{$\beta$-NLL (done):} \code{training.heterosc\_beta} weights the NLL by detached
...
(done):} \code{sweep\_heterosc\_foundation.sh} (same 22-proc leave-uug-out recipe as base22,
...
\paragraph{Resolved --- the coupled fit fails for a structural reason; two-stage $\sigma$ works.}
...
broken --- they must not be trained \emph{jointly} with $\mu$. NAMING TRAP recorded: Seitzer's
...
adaptive importance sampling (VEGAS/foam) \emph{guided by} $|\mathcal{M}|^2$ --- crucially toward
...
\paragraph{L0 validated on the $ee\to u\bar u$ $Z$ resonance --- by GENERATION, not resampling.}
...
\paragraph{The generated fraction sweep --- a clean interior optimum; pure flat over-corrects.}
...
$\log|\mathcal{M}|^2$ (per-dataset standardization already handles the scale --- the earlier
...
\paragraph{L1 --- $\sigma$-reweighting the coverage base does NOT robustly help (3-seed).} On the $f{=}0.25$
...
subsample $400$k \emph{without replacement} by $\pi\!\propto\!\sigma^\alpha$ (no ceiling --- fresh pool).
...
--- the runtime imports the core modules from the main repo, so a worktree edit to \code{train()}
