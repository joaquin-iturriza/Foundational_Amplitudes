...
#SBATCH --array=0-1
#SBATCH --output=compare_models/mass_test_%A_%a.out
#SBATCH --error=compare_models/mass_test_%A_%a.err
#
...
PROJ=$PWD
export AMP_TRAIN_CACHE_DIR=$SCRATCH/amp_cache_masstest
export AMP_FROZEN_DIR=$SCRATCH/datasets_masstest

if [ "$SLURM_ARRAY_TASK_ID" = 0 ]; then N=off;  M=false; else N=mass; M=true; fi
RUNDIR="$PROJ/compare_models/_mass_test/$N"; rm -rf "$RUNDIR"
echo "### mass test $N (mass_from_momenta=$M) ###"
python run.py model=lloca local=none \
  data.source=recipes "data.processes_file=${PROJ}/recipes/mass_threshold_test.yaml" \
