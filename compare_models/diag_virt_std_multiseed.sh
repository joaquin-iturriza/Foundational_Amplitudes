...
RES="$PROJ/compare_models/multiseed25/diag_virt_std_seed${SEED}.json"
echo "=== diag seed=$SEED  lr=$LR reg=$REG ema=$EMA  $(date +%H:%M:%S) ==="
rm -rf "$PROJ/runs/dvstdms/s${SEED}"
python run.py model=lloca local=none \
  ...
  exp_name="dvstdms_s${SEED}" run_dir="$PROJ/runs/dvstdms/s${SEED}"
