...
#SBATCH --array=0-4%5
#SBATCH --output=compare_models/dvirtms_%A_%a.out
#SBATCH --error=compare_models/dvirtms_%A_%a.err
#
...
python run.py model=lloca local=none \
  ...
  exp_name="dvirtms_s${SEED}" run_dir="$PROJ/runs/dvirtms/s${SEED}"
