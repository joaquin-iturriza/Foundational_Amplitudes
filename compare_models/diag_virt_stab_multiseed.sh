...
#SBATCH --array=0-4%5
#SBATCH --output=compare_models/dvstabms_%A_%a.out
#SBATCH --error=compare_models/dvstabms_%A_%a.err
#
...
read LR REG WARM ETA EMA < <(python -c "import json;h=json.load(open('compare_models/encab25_best_hps.json'))['combo']['hp'];print(h['training.lr']/3.0,h['training.regularization_lambda'],h['training.cosanneal_warmup_frac'],h['training.cosanneal_eta_min'],h['training.ema_decay'])")

mkdir -p compare_models/multiseed25
RES="$PROJ/compare_models/multiseed25/diag_virt_stab_seed${SEED}.json"
echo "=== diag seed=$SEED  lr=$LR reg=$REG ema=$EMA  $(date +%H:%M:%S) ==="
rm -rf "$PROJ/runs/dvstabms/s${SEED}"
python run.py model=lloca local=none \
  ...
  exp_name="dvstabms_s${SEED}" run_dir="$PROJ/runs/dvstabms/s${SEED}"
