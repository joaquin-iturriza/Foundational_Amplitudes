...
#SBATCH --array=0-4%5
#SBATCH --output=compare_models/diagms_%A_%a.out
#SBATCH --error=compare_models/diagms_%A_%a.err
#
...
RES="$PROJ/compare_models/multiseed25/diag_seed${SEED}.json"
...
python run.py model=lloca local=none \
  ...
  exp_name="diagms_s${SEED}" run_dir="$PROJ/runs/diagms/s${SEED}"
