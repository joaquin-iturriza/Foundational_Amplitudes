...
if [ "$SLURM_ARRAY_TASK_ID" = 0 ]; then N=off; IMS=false; else N=imass; IMS=true; fi
RUNDIR="$PROJ/compare_models/_imz_test/$N"; rm -rf "$RUNDIR"
...
python run.py model=lloca local=none \
  ...
  data.internal_mass_scalars=$IMS "data.internal_mass_pdgs=[23]" \
