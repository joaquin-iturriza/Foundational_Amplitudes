...
# propagator mass (model.diagram_scanned_mass). If scanned flattens the U, the
# diagram encoder learns the (virtuality × mass) off-shellness LOCALLY (mass on the
# propagator node, virtuality on the incident edge) where the main transformer's
# scalar couldn't. Per-dataset standardization.
...
if [ "$SLURM_ARRAY_TASK_ID" = 0 ]; then N=stale; SM=false; else N=scanned; SM=true; fi
RUNDIR="$PROJ/compare_models/_dmass_test/$N"; rm -rf "$RUNDIR"
echo "### diagram-mass test $N (diagram_scanned_mass=$SM) ###"
python run.py model=lloca local=none \
  ...
  data.internal_mass_scalars=false "data.internal_mass_pdgs=[23]" \
  model.use_diagrams=true model.use_diagram_virtuality=true model.diagram_scanned_mass=$SM \
  ...
  exp_name="dmass_$N" run_dir="$RUNDIR" \
