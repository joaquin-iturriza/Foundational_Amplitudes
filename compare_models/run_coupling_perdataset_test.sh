...
#SBATCH --array=0-1
#SBATCH --output=compare_models/coup_pd_%A_%a.out
#SBATCH --error=compare_models/coup_pd_%A_%a.err
#
...
RUNDIR="$PROJ/compare_models/_coup_test_perdataset/$N"; rm -rf "$RUNDIR"
echo "### coupling test $N (coupling_scalars=$C) ###"
python run.py model=lloca local=none \
  data.source=recipes "data.processes_file=${PROJ}/recipes/coupling_capability_test.yaml" \
