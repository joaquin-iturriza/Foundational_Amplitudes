...
RUNDIR="$PROJ/compare_models/_smoke_cut"; rm -rf "$RUNDIR"
...
python run.py model=lloca local=none \
  ...
  training.iterations=5000 training.validate_every_n_steps=500 \
  training.get_ID=false training.dtype=float32 use_mlflow=false plot=true \
  exp_name="smoke_cut" run_dir="$RUNDIR" \
