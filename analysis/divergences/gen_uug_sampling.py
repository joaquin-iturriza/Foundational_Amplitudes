...
def build(events, sqrts):
    ...
    with mp.CppDriverPipe(f"{STANDALONE}/{mp.CPP_DRIVER_NAME}", STANDALONE) as pipe:
        perm = mp.row_to_slot_perm(PDG, mp.PROCESSES["ee_uug"]["mg5_generate"])   # stored rows -> MadGraph slots; fixed card alpha_s as before
        me2 = pipe.compute(events, perm=perm)
