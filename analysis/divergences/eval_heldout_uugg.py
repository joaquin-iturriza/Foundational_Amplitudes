...
def main():
    ...
    ap.add_argument("--heldout", default=os.path.join(REPO, "analysis/divergences/heldout_uugg_deepIR.npz"))
    ...
    # preprocess the held-out momenta+amps with THIS run's frozen stats, forward, collect predictions.
    me2 = rows[:, -1]
    ...
    with torch.no_grad():
        pred_prepd, truth_prepd, sigma = exp._collect_predictions(loader)

    # Predicted log|M|^2: the amp target is [log, standardization], so UNDO ONLY the standardization to
    # stay in log space (comparison is on Δlog|M|^2). true_logamp = log(|M|^2) directly.
    ...
    for lo, hi in DECADES:
        ...
        print(f"  [{lo:.0e},{hi:.0e})   {int(m.sum()):>7d} {err2[m].mean():>12.4e} {np.sqrt(err2[m].mean()):>8.4f}",
              flush=True)
    # save row-aligned for downstream plotting/comparison
